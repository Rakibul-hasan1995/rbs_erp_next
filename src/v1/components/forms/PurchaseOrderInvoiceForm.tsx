...
import { OrderExpand, PurchaseOrdersExpand } from "@/v1/utils/Types";
import {
   Box,
   Button,
   FormControl,
...
const PurchaseOrderInvoiceForm = ({ submit, initialValues }: Props) => {

   const [orders, setOrders] = useState<PurchaseOrdersExpand[]>([])

   const fetchOrders = async (id: string) => {
      try {
         const { data } = await Axios.get(`/api/v1/users/${id}/purchase-orders?limit=50&search_by=status&search=Completed`)
         setOrders(data.data)
      } catch (error) {
         console.log(error)
      }
   ...
   const onSubmit = async (formData: any) => {
      try {
         ...
         formData.cover_photo = fields?.[0].order.cover_photo._id
         formData.supplier = formData.customer.value
   ...
   const calculateTotalUsd = (order: PurchaseOrdersExpand) => {
      const { qty, rate, order: Or } = order
      const {currency} = Or
      if (currency == 'USD') {
   ...
   const calculateTotalBDT = (order: PurchaseOrdersExpand) => {
      const { qty, rate, order: Or } = order
      const { currency } = Or
      const exchange_rate = +order?.supplier?.exchange_rate || 85
      if (currency == 'USD') {
                        ...
                        {orders.map((item) => (
                           <MenuItem key={item._id} value={item._id}>
                              <Box display={'flex'} gap={3} py={1} justifyContent={'center'}>
                                 <Image src={item.order.cover_photo.href} width={40} height={28} alt={item.order.order_name} />
                                 <Typography>{item.order.program_name} {item.order.order_name}</Typography>
                              </Box>
                           </MenuItem>
                        ))}
                        ...
                        <TableBody>
                           {fields.map((item, index) => (
                              <TableRow key={item.id}>
                                 ...
                                 <TableCell align="center" sx={{ border: 1, borderColor: 'darkgrey', }}><Image src={item.order.cover_photo.href} height={40} width={55} alt={item._id} /></TableCell>
                                 <TableCell align="right" sx={{ border: 1, borderColor: 'darkgrey', }}>{item.qty}</TableCell>
                                 <TableCell align="right" sx={{ border: 1, borderColor: 'darkgrey', }}>{item.order.currency} {numberWithCommas(item.rate)}</TableCell>
...
export default PurchaseOrderInvoiceForm